...
namespace FZ4P
{
    public partial class F_Main : Form, IDiaLogReturnData
    {
        ...
        private void F_Main_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                string afVer = Current.AFPidPath.Substring(Current.AFPidPath.Length - 5, 1);
                string oisVer = Current.XPidPath.Substring(Current.XPidPath.Length - 5, 1);
                this.Text += $"_AF_PID_Ver{afVer}_OIS_PID_Ver{oisVer}";
                Process.AfVer = int.Parse(afVer);
                Process.OisVer = int.Parse(oisVer); 
            }
            catch
            {
                MessageBox.Show($"Please Check PID File Name..");
                this.Text += $"_AF_PID_Ver{Process.AfVer}_OIS_PID_Ver{Process.OisVer}";
            }
       ...
       //     SpecGrid.Columns[0].Name = "Axis";
        ...
        private void SetAFPIDUpdate_Click(object sender, EventArgs e)
        {
            var param = new OpenDialogParams()
            {
                ...
                InitialDirectory = Rcp.AfPidSet?.InitDir,
                Filter = "",
        ...
        public void DiaLogResultFileName(string fileFullName, int index = 0)
        {
            if (index == 0)
            {
                Rcp.AfPidSet.Read(fileFullName);
                Current.AFPidPath = Rcp.AfPidSet.FilePath;
                DataIO.SerializeToXMLFile(Current, STATIC.CurrentPath);
                AFPidSetPath.Text = Current.AFPidPath;
            }
            else if (index == 1)
